...
export default function Collapsible(props) {
  ...
  if (props.sectionName === "Executives") {
    ...
    const communications = candidates.filter(
      (candidate) => candidate.position === "Vice President Communications"
    );
    // const studentlife = candidates.filter(candidate => candidate.position === 'Vice President Student Life');
    const academics = candidates.filter(
      (candidate) => candidate.position === "Vice President Academics"
    );
    ...
    allExecutives = [academics, communications];
  ...
  return (
    <div className="py-2" ref={parent}>
      <button className="faq w-full py-2 sticky" onClick={toggle}>
        <div
          className={`question text-left flex justify-between items-center py-2 ${
            isVisible ? "bg-highlight-blue rounded-sm" : "bg-background"
          }`}
        >
          <p className="text-xl mx-2 lg:text-2xl xl:text-3xl font-semibold">
            {props.sectionName}
          </p>
          <div className="mx-2">
      ...
      {isVisible && (
        <div className="answer text-left mx-4 py-2 text-foreground lg:text-lg">
          {props.sectionName === "Executives" ? (
            <CandidateSection role="Executives" candidates={allExecutives} />
          ) : (
            <CandidateSection role="Directors" candidates={allDirectors} />
          )}
        </div>
      )}
      <hr />
