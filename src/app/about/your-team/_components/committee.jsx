...
export default function Committee({ fileName, group }) {
  ...
  const {
    ...
    loading: loadingData,
    error: errorData,
  } = useContentItem("committees", fileName);
    ...
    <div className="grid gap-5 grid-cols-1 mx-12 lg:mx-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 pt-4 items-center justify-center">
      {members.map((item, index) => (
        <div key={index} className="w-full h-full flex flex-col">
          <div className="w-full aspect-square relative overflow-hidden rounded-full shadow-md">
            <Image
              overrideSrc={getImageUrl(
                `about/${item.image ? `your-team/committees/${fileName}/${item.image}` : "Blank.jpg"}`,
              )}
              alt={item.name}
              loading="lazy"
              placeholder="blur"
